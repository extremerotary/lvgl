                                    ...
                                    int32_t needle_length, int32_t value)
{
    ...
    /* Move/resize the needle_line object to the computed AABB */
    /* Important: this is relative to parent (scale) coordinates */
    ...
    if (lv_line_is_point_array_mutable(needle_line) && lv_line_get_point_count(needle_line) >= 2) {
        ...
        /* Look for previously attached buffer (iterate forwards or backwards safely) */
    ...
    /* Done. LVGL will invalidate precisely the rectangle corresponding to needle_line (the AABB) */
}
